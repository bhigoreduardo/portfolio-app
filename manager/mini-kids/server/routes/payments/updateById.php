<?php
  ...
  try {
    $query = "";
    if ($_POST['status'] == 1) {
      $query = "UPDATE payments SET supply=:supply, deadlineAt=:deadlineAt, paymentAt=:paymentAt, amount=:amount, paymentMethod=:paymentMethod, status=:status, obs=:obs WHERE id=:id";
    } else {
      $query = "UPDATE payments SET supply=:supply, deadlineAt=:deadlineAt, amount=:amount, paymentMethod=:paymentMethod, status=:status, obs=:obs WHERE id=:id";
    }
