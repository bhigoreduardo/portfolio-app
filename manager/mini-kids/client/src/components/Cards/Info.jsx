...
const CardInfo = ({ title, value, color, percentage, description, iconColor, icon }) => {
  return (
    <div className="relative flex flex-col p-3 min-w-0 bg-white shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
        <div className="flex flex-row -mx-3">
          <div className="flex-none w-2/3 max-w-full px-3">
            <div>
              <p className="mb-0 font-sans text-sm font-semibold leading-normal uppercase dark:text-white dark:opacity-60 truncate">{title}</p>
              <h5 className="mb-2 font-bold dark:text-white">{value}</h5>
              <p className="mb-0 dark:text-white dark:opacity-60 truncate">
