...
const Dashboard = () => {
  ...
  const removeSale = async () => {
    api.get(`/sales/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getSales = async () => {
    const { data } = await api.get(`/sales/search?search=${search}&page=${page}&limit=${limit}`);
    setSales(data);
  }
  useEffect(() => {
    getSales();
  ...
  return <Layout>
                ...
                ? (
                  ...
                  <div className="p-0 overflow-x-auto">
                    <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                      ...
                      <tbody>
                        {
                          sales.records.map((item, i) => (
                            <tr key={i}>
                              <td className={`${i === sales.records.length - 1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <div className="flex px-2 py-1">
                                  <div>
                                    <img src={item.customerImage ? `${import.meta.env.VITE_SERVER_URL}/images/${item.customerImage}` : "/img/noavatar.png"} alt={item.customer} className="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-9 w-9 rounded-xl" />
                                  </div>
                                  <div className="flex flex-col justify-center">
                                    <h6 className="mb-0 text-sm leading-normal dark:text-white">{item.customer}</h6>
                                    <p className="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{item.customerEmail}</p>
                                  </div>
                                </div>
                              </td>
                              <td className={`${i === sales.records.length - 1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <p className="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{item.saller}</p>
                                <p className="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{new Date(item.createdAt).toLocaleString()}</p>
                              </td>
                              <td className={`${i === sales.records.length - 1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <p className="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{currencyPrice.format(item.amount)}</p>
                                <p className="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{item.payment}</p>
                              </td>
                              {/* <td className={`${i === sales.records.length - 1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                              ...
                              <td className={`${i === sales.records.length - 1 ? "border-b-0" : "border-b"} p-2 text-sm leading-normal text-center align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <span className={`${item.status ? "from-emerald-500 to-teal-400" : "from-slate-600 to-slate-300"} bg-gradient-to-tl px-2.5 text-xs rounded-1.8 py-1.4 inline-block whitespace-nowrap text-center align-baseline font-bold uppercase leading-none text-white`}>{item.status ? "Pago" : "Pendente"}</span>
                              </td>
                              <td className={`${i === sales.records.length - 1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <a title="Imprimir" href={`/vendas/imprimir/${item.id}`} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 mr-2"><i className="fas fa-file-pdf dark:text-white text-slate-700"></i></a>
                                <a title="Editar" href={`/vendas/${item.id}`} className="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 mr-2"><i className="fas fa-user-edit text-sky-400"></i></a>
                  ...
                  <TableButton page={page} setPage={setPage} limit={limit} count={sales.count} />
            </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir esta venda?" setShowModal={setShowModal} callback={removeSale} />}
  </Layout>;
};
