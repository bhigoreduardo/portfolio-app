...
const Dashboard = () => {
  ...
  const removeCustomer = async () => {
    api.get(`/customers/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getCustomers = async () => {
    const { data } = await api.get(`/customers/search?search=${search}&page=${page}&limit=${limit}`);
    setCustomers(data);
  }
  useEffect(() => {
    getCustomers();
  ...
  return <Layout>
                ...
                ? (
                  ...
                  <div className="p-0 overflow-x-auto">
                    <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                      ...
                      <tbody>
                        {
                          customers.records.map((item, i) => (
                            <tr key={i}>
                              <td className={`${i === customers.records.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <div className="flex px-2 py-1">
                                  <div className="w-1/4">
                                    <img src={item.image ? `${import.meta.env.VITE_SERVER_URL}/images/${item.image}` : "/img/noavatar.jpg"} className="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-9 w-9 rounded-xl" alt="user1" />
                                  </div>
                              ...
                              <td className={`${i === customers.records.length -1 ? "border-b-0" : "border-b"} p-2 text-center align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                  ...
                  <TableButton page={page} setPage={setPage} limit={limit} count={customers.count} />
                  </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir este cliente?" setShowModal={setShowModal} callback={removeCustomer} />}
  </Layout>;
};
