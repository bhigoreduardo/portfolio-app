...
const Form = () => {
  ...
  const getCustomer = async () => {
    const { data } = await api.get(`/customers/findById?id=${id}`);
    setCustomer(data);
  }
  useEffect(() => {
    if (id) getCustomer();
  ...
  const customerInitialValues = {
    personType: customer.personType || 1,
  ...
  const handleSubmit = async values => {
    ...
    if (id) {
      ...
      api.post("customers/updateById", formData)
        .then((res) => {
          console.log(res);
      ...
      api.post("/customers/save", formData)
        .then((res) => {
          if (res.status === 500) return toast.error(res.data.message);
          toast.success(res.data.message);
        })
        ...
        <div className="w-full max-w-full px-3 shrink-0 md:flex-0">
          <div className="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            ...
            <div className="flex-auto p-6">
              ...
              <p className="leading-normal uppercase dark:text-white dark:opacity-60 text-sm"><i className="fa fa-image"></i>&nbsp;Foto do cliente</p>
              <div className="flex flex-wrap py-3">
                <div className="w-full max-w-full px-3 shrink-0 md:flex-0">
