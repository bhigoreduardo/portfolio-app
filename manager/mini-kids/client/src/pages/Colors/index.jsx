...
const Dashboard = () => {
  ...
  const removeColor = async () => {
    api.get(`/colors/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getColors = async () => {
    const { data } = await api.get(`/colors/search?search=${search}&page=${page}&limit=${limit}`);
    setColors(data);
  }
  useEffect(() => {
    getColors();
  ...
  return <Layout>
      ...
      <div className="flex-none w-full max-w-full px-3">
        <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          ...
          <div className="flex-auto px-0 pt-0 pb-2">
            ...
            {
              !colors?.records?.length > 0
                ? (
                  <h6 className="dark:text-white text-center mb-6">Nenhuma cor encontrada</h6>
                ) : (
                  <>
                    {/* TABLE */}
                    <div className="p-0 overflow-x-auto">
                      <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                        ...
                        <tbody>
                          {
                            colors.records.map((item, i) => (
                            <tr key={i}>
                              <td className={`${i === colors.records.length - 1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <div className="flex px-2 py-1">
                                  <div className="w-1/8">
                                    <div className="inline-flex items-center justify-center mr-4 transition-all duration-200 ease-in-out h-9 w-9 rounded-xl border border-slate-700" style={{ backgroundColor: item.hex }} />
                                  </div>
                              ...
                              <td className={`${i === colors.records.length - 1 ? "border-b-0" : "border-b"} p-2 text-center align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                    ...
                    <TableButton page={page} setPage={setPage} limit={limit} count={colors.count} />
                  </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir esta cor?" setShowModal={setShowModal} callback={removeColor} />}
  </Layout>;
};
