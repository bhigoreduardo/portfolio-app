...
const Dashboard = () => {
  ...
  const removeCategory = async () => {
    api.get(`/categories/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getCategories = async () => {
    const { data } = await api.get(`/categories/search?search=${search}&page=${page}&limit=${limit}`);
    setCategories(data);
  }
  useEffect(() => {
    getCategories();
  ...
  return <Layout>
    <div className="flex flex-wrap -mx-3">
      <div className="flex-none w-full max-w-full px-3">
        <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <ListTitle heading="Lista de categorias" target="/categorias/cadastro" title="Adicionar categoria" icon="fab fa-buffer" />
          ...
          <div className="flex-auto px-0 pt-0 pb-2">
            <TableSearch setLimit={setLimit} setSearch={setSearch} />
            {
              !categories?.records?.length > 0
                ? (
                  <h6 className="dark:text-white text-center mb-6">Nenhuma categoria encontrada</h6>
                ) : (
                  <>
                  {/* TABLE */}
                  <div className="p-0 overflow-x-auto">
                    <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                      ...
                      <tbody>
                        {
                          categories.records.map((item, i) => (
                            <tr key={i}>
                              <td className={`${i === categories.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                              ...
                              <td className={`${i === categories.length -1 ? "border-b-0" : "border-b"} p-2 text-center align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                              ...
                              <td className={`${i === categories.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <a title="Editar" href={`/categorias/${item.id}`} className="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 mr-2"><i className="fas fa-user-edit text-sky-400"></i></a>
                  ...
                  <TableButton page={page} setPage={setPage} limit={limit} count={categories.count} />
                  </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir esta categoria?" setShowModal={setShowModal} callback={removeCategory} />}
  </Layout>;
};
