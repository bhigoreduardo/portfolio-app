...
const passwordSchema = yup.object().shape({
  currentPassword: yup.string().required("Senha é obrigatório"),
  newPassword: yup.string().required("Nova senha é obrigatório"),
  confirmPassword: yup.string().oneOf([yup.ref('newPassword'), null], 'Senhas devem ser iguais').required("Confirma senha é obrigatório"),
});
...
const Form = () => {
  ...
  const getUser = async () => {
    const { data } = await api.get(`/users/findById?id=${user.id}`);
    setUser(data);
  }
  useEffect(() => {
    if (user.id) getUser();
  ...
  const passwordFormik = useFormik({
    ...
    onSubmit: (values, onSubmitProps) => handlePasswordSubmit(values, onSubmitProps)
  });
  ...
  return (
    <Layout>
      <form className="flex flex-wrap -mx-3 mb-3" onSubmit={passwordFormik.handleSubmit}>
        <div className="w-full max-w-full px-3 shrink-0 md:flex-0">
          <div className="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div className="border-black/12.5 rounded-t-2xl border-b-0 border-solid p-6 pb-0">
              <div className="flex items-center justify-end gap-4">
                <button type="submit" className="inline-block px-8 py-2 mb-4 font-bold leading-normal text-center text-white align-middle transition-all ease-in bg-blue-500 border-0 rounded-lg shadow-md cursor-pointer text-xs tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">Alterar senha</button>
              </div>
            </div>
            ...
            <div className="flex-auto p-6">
              ...
              <div className="flex flex-wrap py-3">
                <div className="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0 mb-3">
                  <PassField id="currentPassword" label="Senha atual" icon="fas fa-link" title="Senha atual" name="currentPassword" placeholder="Informe sua senha" errors={passwordFormik.touched.currentPassword && passwordFormik.errors.currentPassword} onChange={passwordFormik.handleChange} onBlur={passwordFormik.handleBlur} value={passwordFormik.values.currentPassword} />
                </div>
                <div className="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0 mb-3">
                  <PassField id="newPassword" label="Nova senha" icon="fas fa-link" title="Nova senha" name="newPassword" placeholder="Informe a senha" errors={passwordFormik.touched.newPassword && passwordFormik.errors.newPassword} onChange={passwordFormik.handleChange} onBlur={passwordFormik.handleBlur} value={passwordFormik.values.newPassword} />
                </div>
                <div className="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0 mb-3">
                  <PassField id="confirmPassword" label="Confirmar senha" icon="fas fa-shield-alt" title="Confirmar senha" name="confirmPassword" placeholder="Repita a senha" errors={passwordFormik.touched.confirmPassword && passwordFormik.errors.confirmPassword} onChange={passwordFormik.handleChange} onBlur={passwordFormik.handleBlur} value={passwordFormik.values.confirmPassword} />
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div className="relative w-24 h-24 bg-gray-300 flex items-center justify-center rounded-lg overflow-hidden">
                      {!formik.values.image ? "Sem foto" : (
                        <>
                          ...
                          <img src={`${import.meta.env.VITE_SERVER_URL}/images/${formik.values.image}`} className="object-cover"/>
