...
const Dashboard = () => {
  ...
  const removeReceipt = async () => {
    api.get(`/receipts/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getReceipts = async () => {
    const { data } = await api.get(`/receipts/search?search=${search}&page=${page}&limit=${limit}`);
    setReceipts(data);
  }
  useEffect(() => {
    getReceipts();
  ...
  return <Layout>
    <div className="flex flex-wrap -mx-3">
      <div className="flex-none w-full max-w-full px-3">
        <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <ListTitle heading="Lista de contas receber" target="/contas-receber/cadastro" title="Adicionar conta" icon="fas fa-book" />
                ...
                ? (
                  ...
                  <div className="p-0 overflow-x-auto">
                    <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                      ...
                      <tbody>
                        {
                          receipts.records.map((item, i) => (
                            <tr key={i}>
                              <td className={`${i === receipts.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <div className="flex px-2 gap-1 py-1">
                                  <div className="w-1/4">
                                    <img src={item.customerImage ? `${import.meta.env.VITE_SERVER_URL}/images/${item.customerImage}` : "/img/noavatar.png"} className="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-9 w-9 rounded-xl" alt="user1" />
                                  </div>
                                  <div className="flex flex-col justify-center w-3/4">
                                    <h6 className="mb-0 text-sm leading-normal dark:text-white">{item.customerName} {item.customerLastName}</h6>
                              ...
                              <td className={`${i === receipts.length -1 ? "border-b-0" : "border-b"} p-2 text-center align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                              ...
                              <td className={`${i === receipts.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <a title="Editar" href={`/contas-receber/${item.id}`} className="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 mr-2"><i className="fas fa-user-edit text-sky-400"></i></a>
                  ...
                  <TableButton page={page} setPage={setPage} limit={limit} count={receipts.count} />
                </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir esta conta?" setShowModal={setShowModal} callback={removeReceipt} />}
  </Layout>;
};
