...
const Dashboard = () => {
  ...
  const removeBrand = async () => {
    api.get(`/brands/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getBrands = async () => {
    const { data } = await api.get(`/brands/search?search=${search}&page=${page}&limit=${limit}`);
    setBrands(data);
  }
  useEffect(() => {
    getBrands();
  ...
  return <Layout>
          ...
          <div className="flex-auto px-0 pt-0 pb-2">
            <TableSearch setLimit={setLimit} setSearch={setSearch} />
            {
              !brands?.records?.length > 0
                ? (
                  <h6 className="dark:text-white text-center mb-6">Nenhuma marca encontrada</h6>
                ) : (
                  <>
                  {/* TABLE */}
                      ...
                      <tbody>
                        {
                          brands.records.map((item, i) => (
                            <tr key={i}>
                              ...
                              <td className={`${i === brands.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                <a title="Editar" href={`/marcas/${item.id}`} className="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 mr-2"><i className="fas fa-user-edit text-sky-400"></i></a>
                  ...
                  <TableButton page={page} setPage={setPage} limit={limit} count={brands.count} />
                  </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir esta marca?" setShowModal={setShowModal} callback={removeBrand} />}
  </Layout>;
};
