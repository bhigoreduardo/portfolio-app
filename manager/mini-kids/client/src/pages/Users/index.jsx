...
const Users = () => {
  ...
  const removeUser = async () => {
    api.get(`/users/removeById?id=${itemSelected}`).then((res) => {
  ...
  const getUsers = async () => {
    const { data } = await api.get(`/users/search?search=${search}&page=${page}&limit=${limit}`);
    setUsers(data);
  }
  useEffect(() => {
    getUsers();
  ...
  return <Layout>
    <div className="flex flex-wrap -mx-3">
      <div className="flex-none w-full max-w-full px-3">
        <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <ListTitle heading="Lista de usuários" target="/usuarios/cadastro" title="Adicionar usuário" icon="fas fa-user-plus" />
          ...
          <div className="flex-auto px-0 pt-0 pb-2">
            <TableSearch setLimit={setLimit} setSearch={setSearch} />
            {
              !users?.records?.length > 0
                ? (
                  <h6 className="dark:text-white text-center mb-6">Nenhum usuário encontrado</h6>
                ) : (
                  <>
                    {/* TABLE */}
                      ...
                      <table className="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                        <thead className="align-bottom">
                          <tr>
                            ...
                            <th className="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Perfil</th>
                        ...
                        <tbody>
                          {
                            users.records.map((item, i) => (
                              <tr key={i}>
                                <td className={`${i === users.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                ...
                                <td className={`${i === users.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                  <p className="mb-0 text-xs font-semibold leading-tight dark:text-white dark:opacity-80">{item.profile === 1 ? "Vendedor" : "Administrador"}</p>
                                ...
                                <td className={`${i === users.length -1 ? "border-b-0" : "border-b"} p-2 text-center align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                ...
                                <td className={`${i === users.length -1 ? "border-b-0" : "border-b"} p-2 align-middle bg-transparent dark:border-white/40 whitespace-nowrap shadow-transparent`}>
                                  <a title="Editar" href={`/usuarios/${item.id}`} className="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 mr-2"><i className="fas fa-user-edit text-sky-400"></i></a>
                    ...
                    <TableButton page={page} setPage={setPage} limit={limit} count={users.count} />
                </>
                )
            }
          </div>
    ...
    {showModal && <Modal title="Deseja realmente excluir este usuário?" setShowModal={setShowModal} callback={removeUser} />}
  </Layout>;
};
