...
const validationSchema = yup.object().shape({
  ...
  price: yup.number().required("Preço é obrigatório").typeError("Informe apenas números"),
  promotion: yup.number().optional().typeError("Informe apenas números"),
  image: yup.string().required("Imagem é obrigatório"),
  status: yup.bool().required("Status é obrigatório"),
});

const FormProduct = ({ id, initialData, loading, setLoading, toastMessage, action }) => {
  const params = useParams();
  const initialValues = initialData || {
    store: params.storeId, category: "", name: "", description: "", price: "", promotion: "", image: "", status: "",
  };
  ...
  return (
    <form className="space-y-8 w-full" onSubmit={formik.handleSubmit}>
      ...
      <div className="md:grid md:grid-cols-3 gap-8">
        ...
        <Select id="category" label="Categoria" title="Categoria" name="category" placeholder="Sem categoria" data={categories} errors={formik.touched.category && formik.errors.category} onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.category} />
        ...
        <Input id="price" label="Preço" title="Preço" type="number" name="price" placeholder="Informe o preço" errors={formik.touched.price && formik.errors.price} onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.price} disabled={loading} />
        <Input id="promotion" label="Promoção" title="Promoção" type="number" name="promotion" placeholder="Informe a promoção" errors={formik.touched.promotion && formik.errors.promotion} onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.promotion} disabled={loading} />
